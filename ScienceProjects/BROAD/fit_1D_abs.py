...
def gaussian_HaN2_twocomp_ABS(x,flux_Ha_narrow,flux_Ha_broad,N2Ha_narrow,N2Ha_broad,c,redshift_narrow,redshift_broad,sigma_narrow,sigma_broad,redshift_ABS,flux_Ha_ABS,sigma_ABS):
    redshift_broad=redshift_narrow
    x0_Ha_n=(1+redshift_narrow)*6564.633
    x0_Ha_b=(1+redshift_broad)*6564.633    
    ...
    x0_Ha_ABS=(1+redshift_ABS)*6564.633    
    Halpha_ABS=flux_Ha_ABS*(sigma_ABS)**-1 * (2*np.pi)**-0.5*numpy.exp(-(x-x0_Ha_ABS)**2/(2*sigma_ABS**2))    
    ...
    return c+N2b_N+N2b_B+N2r_N+N2r_B  + (1+Halpha_ABS)*(Halpha_B+Halpha_N)
...
for q in [4]:#range(len(IDlist)):
	...
	#sel_include=(obs_wav>6440*(1+thisz))*(obs_wav<6600*(1+thisz))
	...
	onemodel=Model(gaussian_HaN2_twocomp,independent_vars=('x')) #prefix='m1_'
	onemodel.set_param_hint('flux_Ha_narrow',min=0.,max=30.)
	onemodel.set_param_hint('flux_Ha_broad',min=0.,max=150.)	
	onemodel.set_param_hint('N2Ha_narrow',min=0.,max=0.5)
	onemodel.set_param_hint('N2Ha_broad',min=0.,max=0.5)
	onemodel.set_param_hint('c',min=-0.5,max=0.5)
	onemodel.set_param_hint('redshift_narrow',min=thisz-0.012,max=thisz+0.012)
	onemodel.set_param_hint('redshift_broad',min=thisz-0.02,max=thisz+0.02)
	onemodel.set_param_hint('sigma_narrow',min=7.,max=60.)
	onemodel.set_param_hint('sigma_broad',min=30.,max=300.)
	params=onemodel.make_params(flux_Ha_narrow=0.2,flux_Ha_broad=2.,N2Ha_narrow=0.02,N2Ha_broad=0.02,c=0, \
...
#(x,flux_Ha_narrow,flux_Ha_broad,N2Ha_narrow,N2Ha_broad,c,redshift_narrow,redshift_broad,sigma_narrow,sigma_broad
	...
	model=Model(gaussian_HaN2_twocomp_ABS,independent_vars=('x')) #prefix='m1_'  	redshift_ABS,flux_Ha_ABS,sigma_ABS
	model.set_param_hint('flux_Ha_narrow',min=0.,max=20.)
	model.set_param_hint('flux_Ha_broad',min=0.,max=100.)	
	model.set_param_hint('flux_Ha_ABS',min=-100.,max=0.)		
	...
	model.set_param_hint('sigma_narrow',min=7.,max=65.)
	model.set_param_hint('sigma_broad',min=30.,max=300.)
	model.set_param_hint('sigma_ABS',min=1.,max=20.)
	
	params=model.make_params(flux_Ha_narrow=0.2,flux_Ha_broad=2.,N2Ha_narrow=0.0,N2Ha_broad=0.0,c=0, \
redshift_narrow=thisz,redshift_broad=5.01267,sigma_narrow=12.,sigma_broad=70.,redshift_ABS=thisz,flux_Ha_ABS=-0.2,sigma_ABS=7.)
#(x,flux_Ha_narrow,flux_Ha_broad,N2Ha_narrow,N2Ha_broad,c,redshift_narrow,redshift_broad,sigma_narrow,sigma_broad
	...
	im = axinset2.imshow(imageA,cmap='viridis',vmin=-0.006,vmax=0.036,origin='lower',aspect='auto',interpolation='nearest') #vmax was 0.016
	...
	#ax.plot(xxplot,(1+best_z)*mymodel_ABS,lw=1,color='tab:purple',label=r'H$\alpha$ absorption')	
		#DUMMYS
