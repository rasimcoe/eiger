...
SAVE_FOLDER='/scratch/EIGER/extraction/OPTIMAL_PROFILES_STARS/'
CATALOG='/scratch/EIGER/identification/Stars_J0100_Catalog.fits'
...
for q in range(len(IDlist)):
	...
	for module in ['A','B']:
		data=hdu['SCI%s'%module].data
		errdata=hdu['ERR'].data**0.5


		sel_wav=(wav_array>33000)*(wav_array<37000)
		...
		if thisNclumps_Y==1.:
			...
			model.set_param_hint('m1_totflux',min=0.,max=1000)
			model.set_param_hint('m1_sigma',min=0.8,max=8.)
			model.set_param_hint('m1_c',min=-0.5,max=0.5)
			model.set_param_hint('m1_x0',min=21.,max=29.)

			params=model.make_params(totflux=20,c=0,x0=26,sigma=2)
		...
		#result.params['m1_sigma'].value=1.5*result.params['m1_sigma'].value
