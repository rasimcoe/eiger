...
def plot_images(ix, obj, mosaics, gs, cutsize=100, boarder=30):
...
mosaicdir = basedir + 'junk/current_best/'
mosaics = []

HST_filters = ['F606W', 'F775W','F850LP'] 
for FILTER in HST_filters:
    fname = FILTER+'_stack_F356Wref_final_drc_sci.fits'
    hdul = fits.open(mosaicdir + fname)
    mosaics = mosaics + [dict(image=hdul[0].data, FILTER=FILTER)]
JWST_filters = ['F115W','F200W', 'F356W']
for FILTER in JWST_filters:
    fname = 'stack_'+FILTER+'_pipe4_v2_fluxcal_20220831.fits'
    hdul = fits.open(mosaicdir + fname)
    mosaics = mosaics + [dict(image=hdul['SCI'].data, FILTER=FILTER)]

photcat = Table.read(basedir+"photometry/sextractor/J0100_photcat_v2_short.fits") 
basename = workdir+'J0100_photzcat_v2_EAZY_'
#load data
...
plot_images(ix, photcat[ix], mosaics, gs)
...
#plt.ylim(-2E-3,10E-3)
plt.show()
#plt.savefig('/scratch/mruari/EIGER/SED_fitting/EAZY/working/plots/id'+str(ix)+'_chi2_dist.png')
